'use client'

import {
  ComposedChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer,
} from 'recharts'
import type { Invoice } from '@/types/database'

interface Props {
  planCells: Record<string, number>
  invoices:  Invoice[]
  months:    readonly string[]
}

function monthLabel(iso: string) {
  return new Date(iso + 'T12:00:00').toLocaleString('en-SE', { month: 'short' })
}

function fmtK(v: number) {
  return `${Math.round(v / 1000)} k`
}

const SERIES_LABELS: Record<string, string> = {
  recognised:    'Recognised (P&L)',
  invoiced:      'Invoiced',
  expected_cash: 'Expected cash',
}

export function SowCashFlowChart({ planCells, invoices, months }: Props) {
  // invoiced = when invoice is issued (issue_date)
  const invoicedByMonth = new Map<string, number>()
  // expected_cash = when payment is due per terms (due_date), or actual paid_date
  const expectedByMonth = new Map<string, number>()

  for (const inv of invoices) {
    const issueMonth = inv.issue_date.slice(0, 7) + '-01'
    invoicedByMonth.set(issueMonth, (invoicedByMonth.get(issueMonth) ?? 0) + inv.amount_sek)

    const cashDate  = inv.status === 'paid' && inv.paid_date ? inv.paid_date : inv.due_date
    const cashMonth = cashDate.slice(0, 7) + '-01'
    expectedByMonth.set(cashMonth, (expectedByMonth.get(cashMonth) ?? 0) + inv.amount_sek)
  }

  const data = months.map(m => ({
    month:         monthLabel(m),
    recognised:    Math.round((planCells[m] ?? 0) / 1000),
    invoiced:      Math.round((invoicedByMonth.get(m) ?? 0) / 1000),
    expected_cash: Math.round((expectedByMonth.get(m) ?? 0) / 1000),
  }))

  const hasData = data.some(d => d.recognised > 0 || d.invoiced > 0 || d.expected_cash > 0)
  if (!hasData) return null

  return (
    <div>
      <p className="text-[10px] font-bold text-[#9CA3AF] uppercase tracking-widest mb-3">Cash flow vs Recognition</p>
      <ResponsiveContainer width="100%" height={200}>
        <ComposedChart data={data} margin={{ top: 0, right: 0, left: -10, bottom: 0 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#F3F4F6" vertical={false} />
          <XAxis
            dataKey="month"
            tick={{ fontSize: 10, fill: '#9CA3AF' }}
            axisLine={false}
            tickLine={false}
          />
          <YAxis
            tick={{ fontSize: 10, fill: '#9CA3AF' }}
            axisLine={false}
            tickLine={false}
            tickFormatter={fmtK}
          />
          <Tooltip
            formatter={(v, name) => [`${v} kSEK`, SERIES_LABELS[name as string] ?? name]}
            contentStyle={{ fontSize: 11, borderRadius: 8, border: '1px solid #E5E7EB' }}
          />
          <Legend
            formatter={(value) => SERIES_LABELS[value] ?? value}
            wrapperStyle={{ fontSize: 10 }}
          />
          <Bar dataKey="recognised"    fill="#61b5cc" radius={[2, 2, 0, 0]} maxBarSize={16} />
          <Bar dataKey="invoiced"      fill="#A78BFA" radius={[2, 2, 0, 0]} maxBarSize={16} />
          <Bar dataKey="expected_cash" fill="#F59E0B" radius={[2, 2, 0, 0]} maxBarSize={16} />
        </ComposedChart>
      </ResponsiveContainer>
    </div>
  )
}
